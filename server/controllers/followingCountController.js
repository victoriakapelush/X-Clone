...
const saveFollowingCount = async (req, res) => {
  ...
  try {
    ...
    user.profile.totalFollowing = Array.isArray(user.profile.totalFollowing)
      ? user.profile.totalFollowing
      : [];
    otherUser.profile.totalFollowers = Array.isArray(
      otherUser.profile.totalFollowers,
    )
      ? otherUser.profile.totalFollowers
      : [];
    ...
    res
      .status(500)
      .send({ message: "Error saving following count", error: error.message });
  }
};
