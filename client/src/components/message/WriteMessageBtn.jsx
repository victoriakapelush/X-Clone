...
function WriteMessageBtn({ closeWriteMessage }) {
  ...
  const { query, results, loading, handleSearch, handleChange, setQuery, setResults } = useSearchUsers();
    ...
    <div className="popup-container flex-row">
      <div className="black-window flex-column message-popup-container">
        <div className="message-window-header flex-row">
          ...
          <button className="next-btn-message-popup radius" onClick={handleShowConversation}>Next</button>
        </div>
        ...
        {selectedUser ? (
          <div className="flex-row selected-user-container" onClick={() => setSelectedUser(null)}>
            <img src={`http://localhost:3000/uploads/${selectedUser.profile.profilePicture}`} className="selected-user-image"/>
            <div>
          ...
          <div className="create-group-container flex-row" onClick={handleShowCreateGroup}>
            <span>
              <svg className="grp-svg-msg-popup" viewBox="0 0 24 24" aria-hidden="true">
                <g>
        ...
        {results.length > 0 && (
          <div className="flex-column">
            {results.map((user) => {
              return (
                <div key={user._id} className="flex-row dropdown-user-msg-popup" onClick={() => addUser(user)}>
                  <img src={`http://localhost:3000/uploads/${user.profile.profilePicture}`} className="user-search-image-dropdown"/>
                  <div className="flex-column">
                    <span>{user.originalUsername}</span>
                    <span className="grey-color">@{user.formattedUsername}</span>
                  </div>
                </div>
              );
            })}
