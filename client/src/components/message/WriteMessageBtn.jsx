...
function WriteMessageBtn({ closeWriteMessage, handleWriteMessageClick }) {
  const [showCreateGroup, setShowCreateGroup] = useState(false);
  const { query, results, loading, handleSearch, handleChange, setQuery } = useSearchUsers();

  const onChange = (e) => {
    handleChange(e);
    handleSearch(); // Perform search on each change
  };
  ...
  return (
    <div className="popup-container flex-row">
      <div className="black-window flex-column message-popup-container">
        ...
        {results.length > 0 && (
          <div className="flex-column">
            {results.map((user) => {
              return (
                <div key={user._id} className="flex-row dropdown-user-msg-popup">
      ...
      {showCreateGroup && <CreateGroupPopup handleWriteMessageClick={handleWriteMessageClick} closeWriteMessage={closeWriteMessage} />}
    </div>    
  );
}
