...
const useFollow = (initialUser) => {
  const { loggedinUserData, token, formattedUsername } =
  ...
  useEffect(() => {
    if (loggedinUserData && loggedinUserData.profile && initialUser) {
      const followingIndex = loggedinUserData.profile.totalFollowing.indexOf(
        initialUser._id,
      );
  ...
  const handleFollow = async (otherUserId) => {
    try {
      ...
      const followingIndex =
        updatedCurrentUser.profile.totalFollowing.indexOf(otherUserId);
      if (followingIndex === -1) {
      ...
      updatedCurrentUser.profile.following =
        updatedCurrentUser.profile.totalFollowing.length;

      const followersIndex = updatedOtherUser.profile.totalFollowers.indexOf(
        currentUser._id,
      );
      if (followersIndex === -1) {
      ...
      updatedOtherUser.profile.followers =
        updatedOtherUser.profile.totalFollowers.length;
