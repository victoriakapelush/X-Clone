                            ...
                            <div className='flex-row'>
                                ...
                                <div className='flex-column post-box'>
                                    <Link to={`/profile/${post.user.formattedUsername}`} className='link-to-profile'>
                                        <span className='user-name'>{post.user.profile.updatedName}</span> 
                                        <span className='username-name'>@{post.user.formattedUsername} · {post.time}</span>
                                    </Link>
                                    {post.text && (
                                        <p className={`post-text ${isExpanded ? 'expanded' : ''}`} onClick={toggleText}>
                                            {post.text}
                                        </p>
                                    )}
                                    {post.image && (
                                        <img className='post-image' src={`http://localhost:3000/uploads/${post.image}`} alt={`Post ${index + 1}`} />
                                    )}
                                </div>
                            </div>
