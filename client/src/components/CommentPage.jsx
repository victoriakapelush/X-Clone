    ...
    useEffect(() => {
        const getPostWithComments = async () => {
            try {
                ...
                const response = await axios.get(`http://localhost:3000/api/reply/${username}/comment/${replyId}`, {
                    headers: {
            ...
            <div className='connect-center-container flex-column'>
                ...
                <div className='flex-column popup-reply-post'>
                    <div className='flex-row horizontal-line-replies-section comment-hover'>
                        <div className='pic-vertical-line-box flex-column'>
                            {post && post.user && post.user.profile && <img className='profile-pic no-bottom-margin' src={`http://localhost:3000/uploads/${post.user.profile.profilePicture}`} />}
                        ...
                        <div className='reply-summary-post flex-column'>
                            {post && post.user && post.user.profile && 
                                <span>{post?.user?.profile?.updatedName} 
                                    <span className='reply-replying-to'> @{post?.user?.formattedUsername} · {formatPostTime(post?.time)}</span>
                                </span>
                            }
                            {post.text && <span className='reply-post-text'>{post.text}</span>}
                            {post.image && <img className='reply-post-text reply-post-image' src={`http://localhost:3000/uploads/${post.image}`} />}
                            {post.gif && <img className='reply-post-text reply-post-gif' src={post.gif} />}
                            {post && <span className='reply-replying-to'>Replying to <Link to={`/profile/${post?.user?.formattedUsername}`} className='replying-to-blue'> @{post?.user?.formattedUsername}</Link></span>}
                        </div>
                    </div>
                    ...
                    <div className='create-new-post-window-container flex-row'>
                        <Link to={`/profile/${username}`}>
                            {userData && userData.profile.profilePicture ? (
                                <img className='profile-pic' src={`http://localhost:3000/uploads/${userData.profile.profilePicture}`} />
                                ) : (
                        ...
                        <div className='form-container-new-post'>
                            <form onSubmit={handleSubmit}>
                                <textarea className='post-textarea' 
                                 ...
                                 style={{
                                        ...
                                        backgroundImage: imageUrl ? `url(${imageUrl})` : `url(${selectedGif})`, 
                                ...
                                <div className='flex-row button-and-upload-pic'>
                                    <div className='upload-pic-container'>
                                        <button onClick={handleUploadClick} className='hover-effect' data-username='Media'>
                                        ...
                                        <input
                                            ...
                                            style={{ display: 'none' }}
                                        />
                                        <button onClick={() => setShowGifModal(true)} className='hover-effect' data-username='GIF'>
                                            <svg className='upload-pic radius' viewBox="0 0 24 24" aria-hidden="true">
                                                <g>
                            ...
                            {showEmojiPicker && (<EmojiPicker theme={Theme.DARK} style={{ position: 'absolute' }} onEmojiClick={onEmojiClick} />)}                
                        </div>
                    </div>
                </div>
                    {post && post.totalReplies && post.totalReplies.length > 0 ? (
                        post.totalReplies.map((reply) => (
                        <div key={reply._id} className="flex-column comment-hover">
                            <Link to={`/post/${reply.post?.user.formattedUsername}/status/${reply._id}`}>
                                <div className='flex-row  top-border-horizontal'>
                                    ...
                                    <div className='reply-summary-post flex-column'>
                                        ...
                                        {reply.image && reply.image !== null && reply.image !== undefined && (
                                            <img className='reply-post-text reply-post-image' src={`http://localhost:3000/uploads/${reply.image}`} />
                                        )}
                                        {reply.gif && reply.gif !== null && reply.gif !== undefined && (
                                            <img className='reply-post-text reply-post-gif' src={reply.gif}/>
                                        )}
                                    </div>
                                </div>
                            </Link>
                ...
                <div className="save-icons flex-row">
                    ...
                    <div>
                        ...
                        <p className='top-border-horizontal no-comments-yet'>No comments yet.</p>
                    )}
                </div>
            </div>
